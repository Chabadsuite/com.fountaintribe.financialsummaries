<div id='help'>
  <b>Current financial summary for this contact as well as other contacts with whom they have a permissioned relationship</b>
  <br>Your fiscal year started {$fiscal_year_start_date_formatted} which impacts the information shown
</div>
...
<table border=0 cellspacing=0 cellpadding=0 style="border: 0;">
  ...
  {if $count_unpaid_pledges <> "0" }
    ...
    <tr>
      <td>
        <a class='button' href='/civicrm/contribute/transact?reset=1&id={$paybal_page_id}&cid={$cid}' target='_new'>Pay Existing Balance(s) Using Installments or In Full</a>
      </td>
    </tr>
    {/if}
  {/if}
  ...
  <select onchange='if (this.value) window.location.href=this.value'>
    <option value=''>-- select page to use to create a new ACH/Direct Debit schedule --</option>
      {foreach from=$ach_pages key=cur_id item=cur_title $ach_pages}
        ...
        <br>
         <option value='/civicrm/contribute/transact?reset=1&cid={$cid}&id={$cur_id}'>via '{$cur_title}'</option>
        {/if}
      {/foreach}
   </select><br>
{/if}
...
<span id='view_area' style='width: 100%; text-align: left;'><br><br>
  <div class="crm-accordion-wrapper">
    ...
    <div class="crm-accordion-body">
      <div class="no-border form-layout-compressed">
        <form>
          <select id='select_view' onChange="financial_summary_toggle_obligation(this.options[this.selectedIndex]);">
...
<div id='show_to_end_fiscal_year' style='display: none;'>
  ...
  {$values[$cid][$token_balance_long]}<br><br>

  <strong>Amount Due by {$end_date_formatted}: </strong>
 	{$values[$cid][$token_amount_due_end_fiscal_long]}
  {* print table *}
  <br>

  <h3>Obligations Until End of Fiscal Year</h3>
...
<div id='show_to_today' style='display: block;'>
  ...
  <strong>Amount Currently Due: </strong>
...
<div id='show_to_today_exclude_closed_balances' style='display: none;'>
  ...
  {* print table. *}
  <br>
  <h3>Obligations Until Today Excluding Closed Balances</h3>
...
<div id='show_to_today_exclude_after_fiscalyear_closed_balances' style='display: none;'>
  ...
  {* print table. *}
  <br>
  <h3>Obligations Until Today Excluding Closed Balances and Excluding Future Fiscal Years</h3>
